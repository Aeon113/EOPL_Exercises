...
(define-datatype n-exp n-exp?
  ...
  [n-cons-exp (lhs n-exp?) (rhs n-exp?)]
  [n-car-exp (the-list n-exp?)]
  [n-cdr-exp (the-list n-exp?)]
  [n-null?-exp (the-list n-exp?)]
  [n-emptylist-exp]
  [n-unpack-exp (var-count number?) (the-list n-exp?) (body n-exp?)]
  )
...
(define-datatype expval expval?
  ...
  [list-val (the-list (list-of expval?))]
  )
    ...
    (cases Exp exp
      ...
      [cons-exp (lhs rhs)
               (n-cons-exp
                ...
                (translation-of rhs senv))]
      [car-exp (the-list)
              (n-car-exp (translation-of the-list senv))]
      [cdr-exp (the-list)
              (n-cdr-exp (translation-of the-list senv))]
      [null?-exp (the-list)
                (n-null?-exp (translation-of the-list senv))]
      [emptylist-exp () (n-emptylist-exp)]
      [unpack-exp (var-list the-list body)
                 (n-unpack-exp
                  (length var-list)
                  (translation-of the-list senv)
                  (translation-of body (batch-extend-senv var-list senv)))]
      )))

; --- Evaluators ---
(define nameless-unpack-handler
  (lambda (var-count list-of-expval body-nexp nenv)
    (let
        ([check-vals
          (lambda ()
            (cond
              [(not ((list-of expval?) list-of-expval))
               (eopl:error 'nameless-unpack-handler
                          "Cannot unpack from a non-list value:~%~A~%"
                          list-of-expval)]
              [(not (eqv? var-count (length list-of-expval)))
               (eopl:error 'nameless-unpack-handler
                          "Arity Mismatch: Expected: ~A\tGiven: ~A~%with list list-of-expval:~%~A~%"
                          var-count (length list-of-expval) list-of-expval)]))])
      (begin
        (check-vals)
         (value-of
          body-nexp
          (batch-extend-nenv list-of-expval nenv))))))
...
(define value-of
  (lambda (exp nenv)
    (cases n-exp exp
      ...
      [n-cons-exp (lhs rhs)
                 (list-val
                  (cons (value-of lhs nenv)
                  (extract-expval (value-of rhs nenv))))]
      [n-car-exp (the-list)
                (car
                 (extract-expval (value-of the-list nenv)))]
      [n-cdr-exp (the-list)
                (list-val
                 (cdr
                  (extract-expval (value-of the-list nenv))))]
      ...
      [n-emptylist-exp ()
                      (list-val '())]
      [n-unpack-exp (var-count the-list body)
                   (nameless-unpack-handler
                    var-count
                    (extract-expval (value-of the-list nenv))
                    body
                    nenv)]
    ...
    (letrec
        ([aux
          (lambda (remains)
            (if (null? remains)
               '()
               (cons (extract-expval (car remains)) (aux (cdr remains)))))])
      (if (list? val)
         (aux val)
         val))))
        ...
        [a-program (exp)
                  (let ([translation (translation-of exp (empty-senv))])
                    (let ([value (value-of translation (empty-nenv))])
                      (handle-list (extract-expval value))))]))))