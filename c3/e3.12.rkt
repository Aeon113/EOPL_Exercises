    ...
    [let-exp (var val body)
             (evaluate-exp
              ...
              (extend-env
               ...
               env))]
    [cond-exp (lst-1 lst-2)
             (letrec
                 ([aux
                   (lambda (lst-1-remain lst-2-remain)
                         (cond
                           [(null? lst-1-remain)
                            (eopl:error 'evaluate-exp
                                       "No condition is evaluated to true in this statement: ~%~A~%" exp)]
                           [(evaluate-exp (car lst-1-remain) env) (evaluate-exp (car lst-2-remain) env)]
                           [else (aux (cdr lst-1-remain) (cdr lst-2-remain))]))])
               (aux lst-1 lst-2))]
    ))
